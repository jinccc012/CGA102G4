...
<body>
					...
					<div class="col-lg-5">
						...
						<div class="intro">
							...
							<div class="custom-breadcrumbs">
								<a href="<%= request.getContextPath()%>/index">首頁</a> <span class="mx-2">/</span> <strong>中古車列表</strong>
							</div>
						</div>
					</div>
				</div>
			...
			<div class='divunder'>
				...
				<div class='divr'>
					...
					<span class='text-r'>品牌: ${scar.scar_brand}</span><br> <span
						...
						class='text-r'>所在站點: <c:if test="${scar.st_no == 'TC'}">台中</c:if><c:if test="${scar.st_no == 'TPE'}">台北</c:if><c:if test="${scar.st_no == 'KH'}">高雄</c:if></span><br>
					...
					<!-- 預約賞車按鈕 -->
					<input type="button" class="btn btn-warning reservationbtn" value="預約賞車" id="${i.count}"
						...
						<input type="hidden" value="<fmt:formatDate value="${scar.scar_endtime}" pattern="yyyy-MM-dd HH:mm:ss" />">
				</div>
					<!-- 隱藏的預約表單 -->
					<div class="resform" id="reservation${i.count}" style="position: absolute; width: 240px; height: 270px; background-color: #FFF0D4; top: 50px; left: 1000px; z-index: 2; display: none; border: 2px solid #FFBA3B; border-radius: 10px;">
						<form id="reserveform${i.count}">
							...
							<input name="st_no" type="hidden" style="text-align: center; background-color: #FFFAF2; font-weight: 900; margin-left: 20px;" value="${scar.st_no}" readonly>
							<input style="text-align: center; background-color: #FFFAF2; font-weight: 900; margin-left: 20px;" value="<c:if test="${scar.st_no == 'TC'}">台中</c:if><c:if test="${scar.st_no == 'TPE'}">台北</c:if><c:if test="${scar.st_no == 'KH'}">高雄</c:if>" readonly>
							
							<p style="text-align: center; font-weight: 900; margin: 0px;">預約日期:</p>
							<div style="justify-content: center; display: flex;">
								<input name="sr_time" class="f_date1 srtime" type="text" style="background-color: #FFFAF2; font-weight: 900; text-align: center;" onkeydown="return false">
							...
							<div style="justify-content: center; display: flex; margin-top: 10px;">
								...
								<input class="btn btn-warning reservebtn" id="sub" type="button" value="送出" style="width: 80px; height: 30px; padding: 0px;">
							</div>
						</form>
					</div>
			</div>
	...
	<script>
		...
		$(document).on('click', '.reservationbtn', function(e) {
			...
			$('#reservation' + id).show();
			datereserve = new Date(Date.parse($(this).next().val())-$.now()).getDate();
// 			let fy = new Date($.now()).getFullYear();
// 			let mon = new Date($.now()).getMonth();
// 			let da = new Date($.now()).getDate();
...
// 				startDate: fy+'-'+mon+'-'+da,  // 起始日
				minDate : '-1970-01-01', // 去除今日(不含)之前
				maxDate : '+1970-01-0'+datereserve // 去除今日(不含)之後 0+(終止時間減調今天)
			});
		});
		...
		$(document).on('blur','.srtime',function(e){
...
<!-- 	送出預約表單 -->
    ...
    $(document).on('click',".reservebtn",function (e) {
        let formid = $(this).parent().parent().attr('id'); //form表單id
        console.log( new FormData( $(this).parent().parent()[0]));
        $.ajax({
            type: "POST",
            url: "/CGA102G4/scar/scarReserve",
