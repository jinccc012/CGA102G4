					...
					<div class="col-lg-5">
						<div class="intro">
							...
							<div class="custom-breadcrumbs">
								...
								<a href="<%=request.getContextPath()%>/scar/scarAuctionAll">中古車列表</a>
								<span class="mx-2">/</span>
								<strong>中古車競拍</strong>
							</div>
						</div>
		...
		<div class='divunder'>
			...
			<div class="divtext">
				...
				<div class='divr'>
					...
					<span class='text-r'>所在站點: <c:if test="${scarVO.st_no == 'TC'}">台中</c:if><c:if test="${scarVO.st_no == 'TPE'}">台北</c:if><c:if test="${scarVO.st_no == 'KH'}">高雄</c:if></span><br>
...
<!-- 			顯示最高價的人 -->
			<div style="text-align: center;">
				<c:if test="${mebName != null}">
					<p style="color: red; font-weight: 900; font-size: 24px">目前出價最高者為: <span id="topmeb">${mebName}</span></p>
				</c:if>
				<c:if test="${mebName == null}">
					<span style="color: red; font-weight: 900; font-size: 24px">目前無人出價</span>
				</c:if>
			</div>
		</div>
		<!-- 下方區塊end -->
		...
		<div class="laws" style="height: 200px; width: 800px; background-color: #FFFAF2; border: 2px solid #FFBA3B; border-radius:10px ; position:relative; left:360px; top:-80px; color:black; font-family: 微軟正黑體; font-weight: 900;">
			<p>
			&nbsp;&nbsp;拍賣規則:<br>
			&nbsp;&nbsp;1. 出價最低需加價<span style="color:red;">1000</span>，並且出價金額須高於目前競拍之最高金額<br>
			&nbsp;&nbsp;2. 拍賣時不公開底價。<br>
	...
	function connect(){
		...
		webSocket.onmessage = function(event){
			let topmeb = document.getElementById('topmeb');
			...
			let getname = jsonObj.topname; // 最高價的人
			let id = "${scarVO.scar_no}";
			if(jsonObj.id === id && getprice > priceonweb)
				spanprice.innerText = getprice;
				topmeb.innerText = getname;
		}
